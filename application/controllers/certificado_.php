...
Class Certificado extends CI_Controller{
    ...
    */
    function muestra(){
        ...
        $pdf->Image('img/plantillas/certificado_grande(18x14cm)(1062x826px).png',0,0);
            ...
            foreach ($eventos as $evento):                
                if(($i%2)==0){
                    ...
                    if($evento->Certificado_Evento == NULL){
                        //$pdf->Image('img_certificados/'.$id_evento.'/'.$evento->certificado_Evento, 10, 10);
                    }else{
            ...
            foreach ($eventos as $evento):                
                ...
                if($evento->ImpresionCertificadoHorizontal == 1){
                    ...
                    if($evento->Certificado_Evento == NULL){
                        ...
                        $pdf->Image('img_certificados/'.$id_evento.'/'.$evento->Certificado_Evento, 0, 0, 300, 210);
                    }
                    $pdf->SetXY(5,50);
                    $pdf->SetFont('Arial','B',24);
                    $Nombres = $this->pasarMayusculas($asistente->Nombres);                                
                ...
                //Certificado Vertical    
                }else  if($evento->ImpresionCertificadoVertical == 1){
                    if($uy == 0){
                        //Generamos una nueva hoja de ImprimirPDF extension del FPDF
                        $pdf = new ImprimirPDF();
                    }    
                    ...
                    if($evento->Certificado_Evento == NULL){
                        ...
                        $pdf->Image('img_certificados/'.$id_evento.'/'.$evento->Certificado_Evento, 0, 0, 210, 300);
                    }
                    $pdf->SetXY(5,50);
                    $pdf->SetFont('Arial','B',24);
                    $Nombres = $this->pasarMayusculas($asistente->Nombres);                                
        ...
        foreach ($asistentes as $asistente):                                 
            foreach ($eventos as $evento):                                
                    ...
                    if($evento->ImpresionCertificadoHorizontal == 1){
                        ...
                        $Nombres = $this->pasarMayusculas($asistente->Nombres);                                
                        $Apellidos = utf8_decode($asistente->Apellidos);
                        $pdf->MultiCell(290,7,  strtoupper($Nombres),0,'C');//Nombres del participante
                        $pdf->SetFont('Arial','B',12);
                        $pdf->SetX(5);
                        $pdf->MultiCell(290,7, $Apellidos,0,'C');//Apellidos del participante
                    //Certificado Vertical    
                    }else  if($evento->ImpresionCertificadoVertical == 1){
                        //Generamos una nueva hoja de ImprimirPDF extension del FPDF
                        $pdf = new ImprimirPDF();
